...
from ..model import drink_schemas, models, page_schemas
...
def get_drink(db: Session, id: int):
    ...
    if not stored_drink:
        raise HTTPException(status_code=400, detail="This drink doesn't exist")
    return stored_drink
...
def get_user_drinks(db: Session, id: int, page: page_schemas.Page):
...
def create_drink(db: Session, drinkSchema: drink_schemas.DrinkCreate):
    drink = models.Drink(
        ...
        aid=drinkSchema.alcohol.id,
        quantity=drinkSchema.quantity,
        date=datetime.now(),
    )
    db.add(drink)
    db.commit()
